...
def transaction_list(request):
    ...
    total_savings_goal = Decimal('0')
    total_current_savings = Decimal('0')
    for goal in savings_goals:
        total_savings_goal += Decimal(str(goal.target_amount))
        total_current_savings += Decimal(str(goal.current_amount))
    
    total_debt = Decimal('0')
    total_monthly_payments = Decimal('0')
    for loan in loans:
        total_debt += Decimal(str(loan.remaining_amount))
        total_monthly_payments += Decimal(str(loan.monthly_payment))
    
    # вычисление свободных денег
...
def add_loan(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            loan.save()
            messages.success(request, f'Кредит "{loan.name}" успешно добавлен!')
...
def edit_loan(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Кредит "{loan.name}" успешно обновлен!')
            return redirect('budget:loans_list')
    else:
