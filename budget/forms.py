...
class LoanForm(forms.ModelForm):
    class Meta:
        model = Loan
        fields = ['name', 'initial_amount', 'interest_rate', 'paid_amount', 'start_date', 'end_date']
        widgets = {
            'start_date': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}),
            'end_date': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}),
            'name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Название кредита'}),
            'initial_amount': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': '100000', 'step': '0.01'}),
            'interest_rate': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': '12.5', 'step': '0.1'}),
            'paid_amount': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': '0', 'step': '0.01'}),
        }
        labels = {
            'initial_amount': 'Сумма кредита (₽)',
            'interest_rate': 'Процентная ставка (%)',
            'paid_amount': 'Уже выплачено (₽)',
        }